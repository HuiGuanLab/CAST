...
class VisDial(VisionDataset):
    ...
    def __getitem__(self, idx) -> Tuple[Any, Any]:
        ...
        if self.dialog_len == -1:
            dialog_len = random.randint(1, 10)
        else:
            dialog_len = self.dialog_len
        if self._split == "train" and dialog_len >7 and random.random() < 0.2:
            caption = []
        else:
            caption = [data['caption']]
        if self._split == 'train':
            with open(os.path.join(self.recon_path,file_name[:-3]+"txt"), "r") as f:
                l=f.readlines()
